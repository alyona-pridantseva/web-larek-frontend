...
export class AppForm extends Model<IAppForm> {
	...
	setContactsField(field: keyof IContactsForm, value: string) {
		...
		if (this.validateContacts()) {
			this.events.emit('contacts:ready', this.orderPerson);
		}
	}
	...
	setAddressField(field: keyof IAddressForm, value: string) {
		...
		if (this.validateAddress()) {
			this.events.emit('address:ready', this.orderPerson);
		}
	}
	...
	validateAddress() {
		...
		if (!this.orderPerson.address) {
			// console.log(`первое условие валидации адреса: ${this.orderPerson.address}`);
